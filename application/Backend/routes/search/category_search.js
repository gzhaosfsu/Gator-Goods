...
router.get('/', async (req, res) => {
    ...
    try {
        ...
        const query = "SELECT listing.*, product.*, user.* FROM listing JOIN product ON listing.product_id = product.product_id JOIN user ON listing.vendor_id = user.user_id WHERE product.category LIKE ? AND listing.listing_status = 'Active'"
